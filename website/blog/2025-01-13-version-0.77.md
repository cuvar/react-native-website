...
This release ships several features: new styling capabilities such as support for `display: contents`, `box-sizing`, `mixBlendMode`, and `outline`-related properties to provide a more powerful layout options, and Android 16KB page support to be compatible with the newer Android devices. We are also modernizing the community template by migrating it to Swift, but you can keep using your old Objective-C app, if you need to.
...
#### Simplier layouts with `display: contents`
...
The `box-sizing` prop defines how the element's various sizing props (`width`, `height`, `min-width`, `min-height`, etc.) are computed. If `box-sizing` is `border-box`, these sizes apply to the border box of the element. If it is `content-box` they apply to the content box of the element. The default value is `border-box`. The [web documentation](https://developer.mozilla.org/en-US/docs/Web/CSS/box-sizing) is a good source of information if you wish to learn more about how this prop works.

The default value is to use the `border-box` (notice that the default value for React Native is different from the default value on the web).
Have a look at these example: you can see the difference between the two below, one where we use `border-box` and the other where we use `content-box`. Both boxes have `padding: 20` and `borderWidth: 10`, but with `border-box`, we consider border and padding for the sizing; with `content-box`, we consider the content for the sizing.
...
We’ve also introduced `outlineWidth`, `outlineStyle`, `outlineSpread` and `outlineColor`. These outline props work very similar to the respective `border` props, but it is rendered around the **border box** as opposed to around the **padding box**. Check out the [MDN documentation](https://developer.mozilla.org/en-US/docs/Web/CSS/outline) for more details.
...
- [Use a framework](https://reactnative.dev/blog/2024/06/25/use-a-framework-to-build-react-native-apps) such as Expo, with its own dedicated command to create a new project, such as `npx create-expo-app`
...
You don’t have to apply these changes in your app: the Objective-C++ variant of the iOS community template is still supported. New projects will be generated by using Swift as the iOS app language, although you can always migrate back to Objective-C if you need to.
...
React Native core is mostly developed using C++ to encourage code sharing between iOS and Android and other platforms. The interoperability between Swift and C++ is not mature nor stable, yet.
...
We are looking into ways to fill this gap and let you migrate to Swift too.
...
Additionally the several APIs have been removed or restricted in visibility, so they can’t be accessed anymore. Those APIs were internal and not needed to React Native developers directly. You can find the full list below:
